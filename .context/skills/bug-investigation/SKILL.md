...
Reproduce editor bugs through `packages/excalidraw/tests/helpers`. Trace input from `App.tsx` into `Scene`, `Store`, element helpers, and renderers; inspect element versions and `isDeleted` rather than relying only on array length.
