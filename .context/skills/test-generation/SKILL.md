...
Use Vitest and shared `API`, `UI`, `Keyboard`, and `Pointer` helpers. Prefer observable scene/AppState assertions, account for retained deleted elements, and put pure utility tests beside their source while editor workflows live under `packages/excalidraw/tests`.
